...
class network(base_network.base_network):
    ...
    def _build_preprocess(self):
        ...
        if self.args.use_state_embedding:
            self._build_embedding_layers()
        
        else:
            self._state_embedding_layer = self._goal_embedding_layer = \
                tf.identity
            self._embed_goal_size = 2 * self._input_goal_size
    ...
    def _build_outputs(self):
        ...
        if self._is_manager:        
            if self.args.use_delta_state:
                ...
                self._tensor['label_action'] = self._tensor['embed_lookahead']
        ...
        if not self.args.use_state_preprocessing:
            self._tensor['mixture_joint'] = tf.concat(
                [self._tensor['embed_state'], self._tensor['embed_goal']],
                axis = -1
            )
            
            self._tensor['old_mixture_joint'] = tf.concat(
                [self._tensor['embed_state'], self._tensor['embed_old_goal']],
                axis = -1
            )
            
        elif self.args.embed_goal_type == 'linear':
    ...
    def _build_motivations(self):
        ...
        self._tensor['goal_matrix'] = tf.reshape(tf.squeeze(
                tf.extract_image_patches(
        ...
        if not self.args.use_delta_state:
            self._tensor['goal_matrix'] = \
                tf.expand_dims(self._tensor['embed_state'], 2) - \
                self._tensor['goal_matrix'][:,:,:,:]
        ...
        self._tensor['norm_order'] = tf.nn.l2_normalize(
            self._tensor['goal_matrix'][:,:,:-1,:], axis=-1
        )
